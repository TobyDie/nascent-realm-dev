...
import { CTA_URL } from "../ListiclePageV30";
...
export function StickyMobileCta() {
  ...
  return (
    <div className={`v30-stickycta ${show ? "is-show" : ""}`} aria-hidden={!show}>
      <div className="v30-stickycta__inner">
        <div className="v30-stickycta__meta">
          <span className="v30-stickycta__chip">
            <span className="v30-stickycta__chip-label">85% OFF</span>
            <span className="v30-stickycta__chip-timer" aria-live="off">{countdown}</span>
          </span>
          <span className="v30-stickycta__group">
            Next group starts <span className="v30-stickycta__date">{startDate ? fmtShort(startDate) : "Fri, Jun 26th"}</span>
          </span>
          <span className="v30-stickycta__count">
            <span className="v30-stickycta__tick" aria-hidden="true">▲</span>
            <span key={formatted} className="v30-stickycta__num">{formatted}</span>
            <span> women joining this week</span>
          </span>
        </div>
        <a className="v30-stickycta__btn" href={CTA_URL}>Join now <span aria-hidden="true">→</span></a>
      </div>
    </div>
  );
}
