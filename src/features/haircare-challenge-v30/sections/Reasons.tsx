...
function PointerQuoteCard({ quote, name, location, initial, tint }: PointerQuoteCardProps) {
  ...
  return (
    <figure className="v30-pqcard">
      <span className="v30-pqcard__mark" aria-hidden="true">&ldquo;</span>
      <blockquote className="v30-pqcard__quote">{quote}</blockquote>
      <figcaption className="v30-pqcard__foot">
        <span className="v30-pqcard__avatar" style={{ background: t.bg, color: t.fg }} aria-hidden="true">
          {initial}
        </span>
        <span className="v30-pqcard__id">
          <span className="v30-pqcard__name">{name}{location ? `, ${location}` : ""}</span>
        </span>
        <span className="v30-pqcard__stars" aria-label="5 out of 5 stars">★★★★★</span>
      </figcaption>
    </figure>
  );
}
...
function Reason({ eyebrow, number, headline, image, body, quote, band }: ReasonProps) {
  return (
    <section className={`v30-band v30-band--${band} v30-reason`}>
      <div className="v30-container">
        <div className="v30-eyebrow-row">
          <span className="v30-eyebrow-row__label">{eyebrow}</span>
          <span className="v30-eyebrow-row__rule" aria-hidden="true" />
          <span className="v30-eyebrow-row__num">{number}</span>
        </div>
        <h2 className="v30-reason__h">{headline}</h2>
      </div>
      <div className="v30-container v30-reason__imgwrap">
        {image.url ? (
      ...
      <div className="v30-container v30-reason__body">
        {body}
