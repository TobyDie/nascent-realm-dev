...
export function Stars({ className }: { className?: string }) {
  return <span className={`v28-stars ${className ?? ""}`} aria-label="5 out of 5 stars">★★★★★</span>;
}

export function CtaButton({ children, full }: { children: ReactNode; full?: boolean }) {
  return (
    <a className={`v28-cta ${full ? "v28-cta--full" : ""}`} href={CTA_URL}>
      {children}
    </a>
  );
}
...
export function Placeholder({
  ...
  return (
    <img
      className={fullBleed ? "v28-img v28-img--bleed" : "v28-img"}
      src={src}
      width={w}
      height={h}
      alt={alt}
...
export function Marquee({
  ...
  return (
    <div className={`v28-marquee v28-marquee--${variant}`} role="marquee" aria-hidden="true">
      <div className="v28-marquee__track">
        {track.map((t, i) => (
          <span key={i} className="v28-marquee__item">
            {t}
          </span>
        ))}
      </div>
