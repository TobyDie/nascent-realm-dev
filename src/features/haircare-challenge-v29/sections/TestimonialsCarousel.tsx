...
function Card({ t }: { t: Testi }) {
  return (
    <div className="v29-testis__card">
      <div className="v29-testis__imgwrap">
        <img
      ...
      <div className="v29-testis__body">
        <p className="v29-testis__quote">"{t.text}"</p>
        <div className="v29-testis__meta">
          <div className="v29-testis__avatar" aria-hidden="true">{t.initials}</div>
          <div className="v29-testis__who">
            <div className="v29-testis__name">{t.name}</div>
            <div className="v29-testis__ctx">{t.context}</div>
          </div>
          <Stars />
        </div>
      </div>
...
export function TestimonialsCarousel() {
  ...
  return (
    <section className="v29-band v29-band--white v29-testis" id="results">
      <div className="v29-container">
        <h2 className="v29-h2 v29-testis__h">From women who stopped buying and started learning</h2>

        {isMobile ? (
          <>
            <div ref={trackRef} className="v29-testis__track">
              {TESTIS.map((t, i) => (
                <div key={i} className="v29-testis__slide">
                  <Card t={t} />
                </div>
              ))}
            </div>
            <div className="v29-testis__dots">
              {TESTIS.map((_, i) => (
                <button
                  key={i}
                  className={`v29-testis__dot ${i === idx ? "is-on" : ""}`}
                  onClick={() => scrollTo(i)}
          ...
          <div className="v29-testis__grid">
