...
export function Stars({ className }: { className?: string }) {
  return <span className={`v29-stars ${className ?? ""}`} aria-label="5 out of 5 stars">★★★★★</span>;
}

export function CtaButton({ children, full }: { children: ReactNode; full?: boolean }) {
  return (
    <a className={`v29-cta ${full ? "v29-cta--full" : ""}`} href={CTA_URL}>
      {children}
    </a>
  );
}
...
export function Placeholder({
  ...
  return (
    <img
      className={fullBleed ? "v29-img v29-img--bleed" : "v29-img"}
      src={src}
      width={w}
      height={h}
      alt={alt}
...
export function Marquee({
  ...
  return (
    <div className={`v29-marquee v29-marquee--${variant}`} role="marquee" aria-hidden="true">
      <div className="v29-marquee__track">
        {track.map((t, i) => (
          <span key={i} className="v29-marquee__item">
            {t}
          </span>
        ))}
      </div>
