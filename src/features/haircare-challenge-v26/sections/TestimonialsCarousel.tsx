...
function Card({ t }: { t: Testi }) {
  return (
    <div className="v26-testis__card">
      <div className="v26-testis__imgwrap">
        <img
      ...
      {!t.imageOnly && (
        <div className="v26-testis__body">
          <p className="v26-testis__quote">"{t.text}"</p>
          <div className="v26-testis__meta">
            <div className="v26-testis__avatar" aria-hidden="true">{t.initials}</div>
            <div className="v26-testis__who">
              <div className="v26-testis__name">{t.name}</div>
              <div className="v26-testis__ctx">{t.context}</div>
            </div>
            <Stars />
          </div>
        </div>
...
export function TestimonialsCarousel() {
  ...
  return (
    <section className="v26-band v26-band--white v26-testis" id="results">
      <div className="v26-container">
        <h2 className="v26-h2 v26-testis__h">From women who stopped buying and started making</h2>

        {isMobile ? (
          <>
            <div ref={trackRef} className="v26-testis__track">
              {TESTIS.map((t, i) => (
                <div key={i} className="v26-testis__slide">
                  <Card t={t} />
                </div>
              ))}
            </div>
            <div className="v26-testis__dots">
              {TESTIS.map((_, i) => (
                <button
                  key={i}
                  className={`v26-testis__dot ${i === idx ? "is-on" : ""}`}
                  onClick={() => scrollTo(i)}
          ...
          <div className="v26-testis__grid">
